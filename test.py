...
def region_of_interest(img):
    ...
    shape = img.shape
    vertices = np.array(
        [[(0, 0), (shape[1], 0), (shape[1], 0), (6 * shape[1] / 7, shape[0]),
...
#    img = np.copy(img)
    ...
    abs_sobelx = np.absolute(
        sobelx)  # Absolute x derivative to accentuate lines away from horizontal
...
class Line:
    def __init__(self, n=5):
        ...
        self.fit_yvals = np.linspace(0, 100,
                                     num=101) * 7.2  # always the same y-range as image
    ...
    def set_radius_of_curvature(self):
        ...
        if self.avg_fit_coeffs is not None:
            ...
            2 * self.avg_fit_coeffs[0] * y_eval + self.avg_fit_coeffs[
                1]) ** 2) ** 1.5) \
    ...
    def set_line_base_pos(self):
        ...
        self.line_base_pos = (
                             self.line_pos - basepos) * 3.7 / 600.0  # 3.7 meters is about 600 pixels in the x direction
...
def process_image(img):
    ...
    #left_binary = get_binary_lane_image(warped_binary, left, window_center_l,
    #                                    width=300)

    #window_center_r = 940
    if right.detected:
        window_center_r = right.line_pos
    #right_binary = get_binary_lane_image(warped_binary, right, window_center_r,
    #                                     width=300)
